...
// 启用或关闭分析组件
func ComponentToggleHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.ComponentToggleReq
		if err := httpx.Parse(r, &req); err != nil {
			httpx.ErrorCtx(r.Context(), w, err)
			return
		}

		l := components.NewComponentToggleLogic(r.Context(), svcCtx)
		resp, err := l.ComponentToggle(&req)
		if err != nil {
			httpx.ErrorCtx(r.Context(), w, err)
		} else {
